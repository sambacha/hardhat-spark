...
import { ConstantGasPrice, GasPriceType } from './deployment/custom_providers/constant_gas_price';
import { TransactionManager } from './deployment/custom_providers/transaction_manager';
import { GasPriceCalculator, RemoteBucketStorage } from 'ignition-core';
...
const gasPriceProvider = new EthGasStationProvider(ETH_GAS_STATION_API_KEY as string, GasPriceType.average);
...
const txManager = new TransactionManager(provider, wallet, process.env.IGNITION_NETWORK_ID as string, gasCalculator, gasPriceProvider);
