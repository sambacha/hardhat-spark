...
import { buildModule, ContractBinding, sendAfterDeploy } from "ignition-core";
...
export const SynthetixSynths = buildModule(
  "SynthetixSynths",
  async (m: SynthetixModuleBuilder) => {
    const ExchangeRates = m.ExchangeRates;

    for (const { name: currencyKey, subclass, asset } of m.synths as any[]) {
      const TokenStateForSynth = m.bindTemplate(
      ...
      const synthProxyIsLegacy =
        currencyKey === "sUSD" && IGNITION_NETWORK_ID === "1";
      ...
      if (currencyKey === "sUSD") {
        proxyERC20ForSynth = m.bindTemplate(
          `ProxyERC20${currencyKey}`,
          "ProxyERC20",
          m.ETH_ADDRESS
        );
      }
      ...
      sendAfterDeploy(m, TokenStateForSynth, "setAssociatedContract", [Synth], {
        eventName: `afterDeploySynth${currencyKey}`,
      });

      sendAfterDeploy(m, ProxyForSynth, "setTarget", [Synth], {
        eventName: `afterDeploySynthProxyForSynth${currencyKey}`,
      });

      if (proxyERC20ForSynth) {
        sendAfterDeploy(m, Synth, "setProxy", [proxyERC20ForSynth], {
        ...
        sendAfterDeploy(m, Synth, "setProxy", [ProxyForSynth], {
          eventName: `afterDeployProxyERC20ForSynth${currencyKey}`,
        });
      }
