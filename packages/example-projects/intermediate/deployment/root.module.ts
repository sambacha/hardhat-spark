...
export const SomeModule = buildModule(
  "SomeModule",
  async (m: ModuleBuilder, wallets: ethers.Signer[]) => {
    ...
    const ERC20Two = m.bindTemplate(
      "ERC20Two",
      "ERC20",
      "ExampleTokenTwo",
      "EXMPLTWO"
    );
    const Proxy = m.bindTemplate(
      "Proxy",
    ...
    ERC20.afterDeploy(m, "afterDeployMintTokens", async () => {
      ...
      await expectFuncRead(
        ...
        ERC20.deployed().totalSupply
      );
    });

    const mutatorEvent = sendAfterDeploy(m, Proxy, "upgradeTo", [ERC20Two], {
    ...
    m.group(ERC20Two, mutatorEvent).afterDeploy(
      m,
      "afterDeployAndChange",
      async () => {
        const totalSupply = ethers.BigNumber.from(10).pow(17);
        await ERC20Two.deployed().mint(
          await wallets[0].getAddress(),
          totalSupply
        );
