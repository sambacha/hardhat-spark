...
describe('ignition diff - integration', () => {
  ...
  it('should be able to show difference in modules - single new binding', async () => {
    ...
    const projectLocation = path.resolve(rootDir, `./test/projects-scenarios/${projectDir}`);
...
'  └── Contract: Example\n');
  ...
  it('should do nothing if their is no difference in module bindings', async () => {
    ...
    assert.equal(output, 'Nothing changed from last revision - ExampleModule\n');
  });
...
async function runDiffCommand(ignition: IgnitionTests, projectLocation: string, moduleFileName: string = defaultModuleFileName): Promise<void> {
  const deploymentFilePath = path.join(projectLocation, DEPLOYMENT_FOLDER, moduleFileName);
