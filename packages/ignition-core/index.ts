...
import {
  INITIAL_MSG,
  MODULE_NAME_DESC,
} from "./src/services/tutorial/tutorial_desc";
...
export class IgnitionCore {
    ...
    try {
      ...
      for (const [moduleName, moduleFunc] of Object.entries(modules)) {
        ...
        await moduleSession.runAndReturn(async () => {
          await module.init(
            moduleSession,
            ignitionWallets as ethers.Wallet[],
            undefined,
            {
        ...
        let stateFileRegistry = await moduleStateRepo.getStateIfExist(
          moduleName
        );
        for (const moduleStateName of states) {
          const moduleState = await moduleStateRepo.getStateIfExist(
            moduleStateName
          );

          stateFileRegistry = StateResolver.mergeStates(
            stateFileRegistry,
            moduleState
          );
        }
        ...
        const moduleState: ModuleState | null = await moduleResolver.resolve(
        ...
        if (module.getCustomGasPriceProvider() && config.gasPriceProvider) {
          txGenerator.changeGasPriceCalculator(config.gasPriceProvider);
        }

        if (module.getCustomNonceManager() && config.nonceManager) {
          txGenerator.changeNonceManager(config.nonceManager);
        }

        if (module.getCustomTransactionSigner() && config.transactionSigner) {
        ...
        try {
          ...
          await executor.executeModuleEvents(
            moduleName,
            moduleState,
        ...
        const summary = await moduleDeploymentSummaryService.showSummary(
          moduleName,
          stateFileRegistry
        );
    ...
    try {
      ...
      for (const [moduleName, modFunc] of Object.entries(modules)) {
        ...
        if (!checkIfExist(moduleState)) {
          cli.info(
            "Current module state is empty, add something and try again."
          );
          process.exit(0);
        }
      ...
      for (const [moduleName, modFunc] of Object.entries(modules)) {
        ...
        await moduleSession.runAndReturn(async () => {
          await module.init(
            ...
            wallets,
            undefined,
            ignitionConfig as ModuleOptions
          );
        });
  ...
  async usage(
    ...
    for (const [moduleName, moduleFunc] of Object.entries(modules)) {
      ...
      const rawUsage = await moduleUsage.generateRawUsage(
        moduleName,
        stateFileRegistry
      );
      ...
      for (const [elementName, element] of Object.entries(rawUsage)) {
        ...
        if (!checkIfExist(contractAddress)) {
          throw new MissingContractAddressInStateFile(
            `Cannot find deployed contract address for contract: ${elementName}`
          );
        }
      }
  ...
  async tutorial(
    ...
    const yes = await cli.confirm(
      "Are you ready to start? (make sure you have some contracts to start with ;)) (yes/no)"
    );
    if (!yes) {
      return;
    }
  ...
  async migrate(
    ...
    const ignitionStateFiles = stateMigrationService.mapBuildsToStateFile(
      validBuilds
    );

    // store ignition state file
    await stateMigrationService.storeNewStateFiles(
      moduleName,
      ignitionStateFiles
    );

    const moduleStateBindings = await moduleMigrationService.mapModuleStateFileToContractBindingsMetaData(
      ignitionStateFiles
    );
    const moduleFile = await moduleMigrationService.generateModuleFile(
      moduleName,
      moduleStateBindings
    );
...
export async function defaultInputParams(
  ...
  const config = await configService.initializeIgnitionConfig(
    process.cwd(),
    configScriptPath
  );
  ...
  if (config.networks && config.networks[networkName]) {
    if (checkIfExist(config.networks[networkName].rpcProvider)) {
      ...
      process.env.IGNITION_RPC_PROVIDER = String(
        config?.networks[networkName]?.rpcProvider
      );
    }

    if (checkIfExist(config.networks[networkName].blockConfirmation)) {
      process.env.BLOCK_CONFIRMATION_NUMBER = String(
        config.networks[networkName].blockConfirmation
      );
    }

    if (config.networks[networkName].localDeployment) {
    ...
    if (config.networks[networkName].gasPriceBackoff) {
      gasPriceBackoff = config.networks[networkName]
        .gasPriceBackoff as GasPriceBackoff;
    }

    if (checkIfExist(config.networks[networkName].parallelizeDeployment)) {
  ...
  switch (logging) {
    ...
    case Logging.streamlined: {
      let yes = true;
      if (networkName != DEFAULT_NETWORK_NAME && !isLocalDeployment) {
        yes = await cli.confirm(
          "Would you like to be prompted at every single step?"
        );
      }
