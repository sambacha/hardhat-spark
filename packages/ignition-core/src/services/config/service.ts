...
export default class ConfigService implements IConfigService {
  ...
  private config: HardhatIgnitionConfig | undefined;
  ...
  async initializeIgnitionConfig(
    ...
    try {
      ...
      config = Object.entries(configModules)[0][1] as HardhatIgnitionConfig;

      if (!checkIfExist(config)) {
        throw new ConfigMissingError(configScriptPath || CONFIG_SCRIPT_NAME);
      }
    } catch (err) {
      if (err._isUserError) {
        throw err;
  ...
  getAllWallets(rpcPath?: string): ethers.Wallet[] {
    ...
    if (hdPath && mnemonic && checkIfExist(hdPath) && checkIfExist(mnemonic)) {
      for (let i = 0; i < NUMBER_OF_HD_ACCOUNTS; i++) {
        const components: string[] = (hdPath || '').split('/');
  ...
  getFirstPrivateKey(): string {
    ...
    if (this.config?.privateKeys && checkIfExist(this.config?.privateKeys)) {
