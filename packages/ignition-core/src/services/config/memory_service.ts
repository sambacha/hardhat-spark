...
export class MemoryConfigService implements IConfigService {
  ...
  getAllWallets(rpcPath?: string): ethers.Wallet[] {
    ...
    if (
      ...
      if (config.networks[this.networkName]?.mnemonic) {
        mnemonic = config.networks[this.networkName]?.mnemonic || '';
      }
      if (config.networks[this.networkName]?.hdPath) {
  ...
  getFirstPrivateKey(): string {
    ...
    if (
      ...
      this.config.networks[this.networkName] &&
      this.config.networks[this.networkName]?.privateKeys
    ) {
