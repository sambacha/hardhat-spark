...
export class TxExecutor {
    ...
    for (let [elementName, element] of Object.entries(moduleState)) {
      if (checkIfExist((element as ContractBinding)?.bytecode)) {
        ...
        if (checkIfExist(contractAddress)) {
          ...
          this.prompter.alreadyDeployed(elementName);
          await this.moduleStateRepo.storeSingleBinding(element as ContractBinding);
          await this.prompter.finishedBindingExecution(elementName);
          continue;
        }
  ...
  private async executeBatches(moduleName: string, batches: any[], moduleState: ModuleState, registry?: IModuleRegistryResolver, resolver?: IModuleRegistryResolver, moduleConfig?: ModuleConfig) {
    for (const batch of batches) {
      ...
      for (let batchElement of batch) {
        ...
        if (checkIfExist(contractAddress)) {
          ...
          await this.moduleStateRepo.storeSingleBinding(batchElement as ContractBinding);
          continue;
        }
        ...
        try {
          ...
          if (checkIfExist((event.event as ContractEvent)?.deps)) {
            ...
            for (const depName of deps) {
              await this.moduleStateRepo.storeSingleBinding(moduleState[depName] as ContractBinding);
            }
          }
  ...
  private async executeSingleBinding(moduleName: string, binding: ContractBinding, moduleState: ModuleState, parallelized: boolean = false): Promise<ContractBinding> {
    ...
    if (binding?.deployMetaData?.deploymentSpec?.deployFn) {
      ...
      await this.moduleStateRepo.finishCurrentEvent(moduleName, moduleState, `Deploy${binding.name}`);
  ...
  private async sendTransactionAndWait(elementName: string, binding: ContractBinding, signedTx: string): Promise<TransactionReceipt> {
    return new Promise(async (resolve, reject) => {
      try {
        binding.txData = binding.txData as TransactionData;
        const txResp: TransactionResponse = await this.sendTransaction(elementName, binding, signedTx);
